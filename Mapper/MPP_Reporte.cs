...
namespace Mapper
{
    public class MPP_Reporte
    {
        ...
        public List<BE_Pedido> ListarReportes(BE_Reporte reporte)
        {
            ...
            if (reporte.Codigo_Mozo != 0)
            {
                hashtable.Add("@Codigo", reporte.Codigo_Mozo);
            }
            hashtable.Add("@FechaInicio", reporte.FechaInicio);
            hashtable.Add("@FechaFin", reporte.FechaFin);
            DataTable Dt = Acceso.DevolverListado("63 - Reportes_Pedido", hashtable);
            ...
            if (Dt.Rows.Count > 0)
            {
                foreach (DataRow row in Dt.Rows)
                {
                    ...
                    if (!(row[3] is DBNull))
                    {
                        ...
                        hash1.Add("@Codigo", Convert.ToInt32(row[3].ToString()));
                        DataTable Dt1 = Acceso.DevolverListado("24 - Listar_Mesa_Pedido", hash1);
                        if (Dt1.Rows.Count > 0)
                        {
                    ...
                    if (!(row[4] is DBNull))
                    {
                        ...
                        hash2.Add("@Codigo", Convert.ToInt32(row[4].ToString()));
                        DataTable Dt2 = Acceso.DevolverListado("25 - Listar_Mozo_Pedido", hash2);
                        if (Dt2.Rows.Count > 0)
                        {
                    ...
                    DataTable Dt3 = Acceso.DevolverListado("26 - Listar_Plato_Pedido", hash3);
                    if (Dt3.Rows.Count > 0)
                    {
                    ...
                    DataTable Dt4 = Acceso.DevolverListado("27 - Listar_Bebida_Pedido", hash3);
                    if (Dt4.Rows.Count > 0)
                    {
